...
from lib.yolov3.human_detector_realtime import load_model as yolo_model
from lib.yolov3.human_detector_realtime import yolo_human_det as yolo_det
from lib.yolov3.human_detector_realtime import set_model_yolo_det as set_model_yolo_det
...
def set_model(det_dim=416, num_peroson=1, gen_output=False):
    # Updating configuration
    args = parse_args()
    reset_config(args)
    ...
    human_model = yolo_model(inp_dim=det_dim)
    print("L96 cfg" , cfg )
    yolo_det = set_model_yolo_det( human_model, reso=det_dim, confidence=args.thred_score)
    pose_model = model_load(cfg)
    

    return yolo_det, pose_model

def get_keypoints(frame,  track_bboxs, num_peroson, pose_model):
    kpts_result = []
    scores_result = []
    with torch.no_grad():
        # bbox is coordinate location
